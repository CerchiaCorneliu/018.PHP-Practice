<?php
  ...
  class SaleModel extends AbstractModel {
    ...
    public function get(int $saleId): Sale {
      ...
      $query = 'SELECT b.id, b.title, b.author, b.price, sb.amount as stock, b.isbn FROM sale s LEFT JOIN sale_book sb ON s.id = sb.sale_id LEFT JOIN book b ON sb.book_id = b.id WHERE s.id = :id';
      $sth = $this->db->prepare($query);
      $sth->execute(['id' => $saleId]);
